...
**環境変数の 3 層構造と API コンテキスト制約**

SPANK の主要 API は呼べるコンテキストが決まっている（`spank.c` の access check より）:

| API | 有効コンテキスト | 無効時エラー |
|---|---|---|
| `spank_job_control_setenv/getenv()` | LOCAL (srun) / ALLOCATOR (sbatch/salloc) | `ESPANK_NOT_LOCAL` |
| `spank_setenv()` / `spank_getenv()` | **REMOTE のみ** (slurmstepd) | `ESPANK_NOT_REMOTE` |

`init_post_opt` は LOCAL/ALLOCATOR/REMOTE 全てで呼ばれるが、
`spank_setenv()` は REMOTE 専用のため `init_post_opt` 内では使えない。
job control setenv を使い、Slurm に `SPANK_` 自動付加させる方式が正しい。

```
[LOCAL (srun) / ALLOCATOR (sbatch/salloc)]
  spank_job_control_setenv(sp, "RSC_CONFIG", ...)   ← "RSC_*" で格納（SPANK_ なし）
...
[slurmctld: job_submit_rsc.c が spank_job_env から読む]
  ...
  ※ SPANK_RSC_C_EFF は job_submit が spank_job_env に追記する
        |
        | slurmd に渡る（step->env に組み込まれる）
        v
[REMOTE (slurmstepd): slurm_spank_user_init]
  spank_getenv(sp, "SPANK_RSC_*", ...)  ← step->env から読む（REMOTE 専用 API）
  spank_setenv(sp, "SLURM_RSC_*", ...) ← step->env に書く（REMOTE 専用 API、SPANK_ 付加なし）
        |
        v
...
| コンテキスト | 場所 | 動作 |
|---|---|---|
| `S_CTX_LOCAL` | srun | `--rsc` を解析、job control env (`RSC_*`) にセット |
...
ユーザジョブの環境変数は 2 段階で提供される。

**① Slurm が自動的にセット（`SLURM_SPANK_RSC_*`）**  
sbatch/srun が `spank_job_env` を `SLURM_SPANK_<entry>` としてジョブの保存環境変数に追加する
（`src/sbatch/sbatch.c:_set_spank_env()`）。slurmstepd はこの環境変数を `step->env` に取り込むため、
ユーザジョブでは `SLURM_SPANK_RSC_*` が自動的に参照できる。

**② `slurm_spank_user_init()` がコピー・追加（`SLURM_RSC_*` と `OMP_*`）**  
REMOTE context で `spank_getenv(sp, "SLURM_SPANK_RSC_*", ...)` で上記を読み取り、
`spank_setenv()` で以下をセットする（名前はそのまま、`SPANK_` は付かない）。  
**全ジョブが対象**（`--rsc` 省略時もデフォルト値が渡されるためセットされる）。

| 変数名 | 内容 | 備考 |
|---|---|---|
| `SLURM_SPANK_RSC_SPEC` | `--rsc` に渡した未加工の文字列 | Slurm が自動セット。`--rsc` 省略時は `""` |
| `SLURM_SPANK_RSC_P` | プロセス数 | Slurm が自動セット |
| `SLURM_SPANK_RSC_T` | プロセスあたりスレッド数 | Slurm が自動セット |
| `SLURM_SPANK_RSC_C` | 指定コア数（c' 適用前） | Slurm が自動セット |
| `SLURM_SPANK_RSC_M` | プロセスあたりメモリ (MiB) | Slurm が自動セット |
| `SLURM_SPANK_RSC_G` | GPU 数 | Slurm が自動セット（GPU モード時のみ） |
| `SLURM_RSC_SPEC` | 上記のエイリアス | user_init がコピー |
| `SLURM_RSC_P` | 上記のエイリアス | user_init がコピー |
| `SLURM_RSC_T` | 上記のエイリアス | user_init がコピー |
| `SLURM_RSC_C` | 上記のエイリアス | user_init がコピー |
| `SLURM_RSC_M` | 上記のエイリアス | user_init がコピー |
| `SLURM_RSC_G` | 上記のエイリアス | user_init がコピー（GPU モード時のみ） |
| `SLURM_RSC_C_EFF` | 実効コア数（c' 適用後） | `SLURM_CPUS_PER_TASK` から転写 |
...
GPU モードの場合は `SLURM_RSC_SPEC` / `SLURM_RSC_G` のみがセットされ、`SLURM_RSC_P/T/C/M/C_EFF` および `OMP_*` はセットされない。
